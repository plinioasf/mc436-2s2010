...
\centering\begin{tikzpicture}[mindmap,
  ...
  % Applied area:Pos conferencia (90 a 270 graus)

  \begin{scope}[mindmap, concept color=green!50!black,text=white]
    \node [concept] at (14.0,-14) {P\'os-Confer\^encia} 
      child [grow=90, level distance=120] 
        {node [concept] (pogf) {Galeria de fotos}}
    child [grow=180] 
        {node [concept] (pofd) {Feedback}}
    child [grow=270] 
        {node [concept] (podp) {Disponibiliza\c{c}\~ao }};
  \end{scope}

  % Applied area:usuários  (one subfield)

  \begin{scope}[mindmap, concept color=violet, text=white,every annotation/.style={fill=black}]
    \node [concept](usr) at (0,-25.0)  {Usu{\'a}rios}
...
%Sistema
  \begin{scope}[mindmap, concept color=brown!50!black, text=white,every annotation/.style={fill=black}]
    \node [concept](stg) at (0,-14.0)  {Sistema gerenciador de confer\^encias tecnol\'ogicas}
      child [grow=0, level distance=120] 
        {node [concept] (ac) {Acessibilidade}}
    child [grow=60, level distance=150] 
        {node [concept] (fa) {Facilidade de uso}}
      child [grow=120, level distance=120] 
        {node [concept] (se) {Segunra\c{c}a}}
      child [grow=180, level distance=120] 
        {node [concept] (ve) {Velocidade}} 
     child [grow=240, level distance=120] 
        {node [concept] (co) {Confiabilidade}}
   child [grow=300, level distance=120] 
        {node [concept] (ro) {Robustez}};
